...
int WatParser::ParseFunctionTypeInner(const Environment& env, Queue<WatToken>& tokens, FunctionType& sig, DebugInfo** info, bool anonymous)
{
  sig.form = TE_func;
  int err;
...
int WatParser::ParseTypeUse(Queue<WatToken>& tokens, varuint32& sig, DebugInfo** info, bool anonymous)
{
  ...
  if(tokens.Size() > 1 && tokens[0].id == TOKEN_OPEN && (tokens[1].id == TOKEN_PARAM || tokens[1].id == TOKEN_RESULT))
  {
    ...
    {
      if(sig < m.type.n_functions && !MatchFunctionType(m.type.functions[sig], func))
        return ERR_WAT_TYPE_MISMATCH;
    }
    else
    {
      sig = m.type.n_functions;
...
int WatParser::ParseConstantOperator(Queue<WatToken>& tokens, Instruction& op)
{
  ...
  {
  case OP_i32_const:
    err = ResolveTokeni32(tokens.Pop(), numbuf, op.immediates[0]._varsint32);
    break;
  case OP_i64_const:
    err = ResolveTokeni64(tokens.Pop(), numbuf, op.immediates[0]._varsint64);
    break;
  case OP_f32_const:
    err = ResolveTokenf32(tokens.Pop(), numbuf, op.immediates[0]._float32);
    break;
  case OP_f64_const:
    err = ResolveTokenf64(tokens.Pop(), numbuf, op.immediates[0]._float64);
    break;
...
int WatParser::ParseOperator(Queue<WatToken>& tokens, Instruction& op, FunctionBody& f, FunctionType& sig, WatParser::DeferWatAction& defer)
{
  ...
  case OP_local_set:
  case OP_local_tee:
    op.immediates[0]._varuint32 = GetLocal(f, sig, tokens.Pop());
    if(op.immediates[0]._varuint32 == (varuint32)~0)
  ...
  case OP_global_set:
  case OP_call:
    defer = WatParser::DeferWatAction{ op.opcode, tokens.Pop(), 0, 0 };
    break;
  case OP_i32_const:
  case OP_i64_const:
  case OP_f32_const:
...
int WatParser::ParseExpression(Queue<WatToken>& tokens, FunctionBody& f, FunctionType& sig, varuint32 index)
{
  ...
  case TOKEN_IF:
  {
...

    {
      ...
      op.line = t.line;
      op.column = t.column;
      if(err = AppendArray<Instruction>(env, op, f.body, f.n_body)) // We append the if instruction _after_ the optional condition expression
        return err;
    }
  }
  ...
  default:
  {
    ...
    if(err = ParseOperator(tokens, op, f, sig, defer))
      return err;
...
int WatParser::ParseInstruction(Queue<WatToken>& tokens, FunctionBody& f, FunctionType& sig, varuint32 index)
{
  int err;
  varsint7 blocktype;
  switch(tokens[0].id)
...
int WatParser::InlineImportExport(const Environment& env, Module& m, Queue<WatToken>& tokens, varuint32* index, varuint7 kind, Import** out)
{
  int err;
...
int WatParser::ParseLocalAppend(const Environment& env, FunctionBody& body, Queue<WatToken>& tokens)
{
...
int WatParser::ParseFunction(Queue<WatToken>& tokens, varuint32* index, StringRef name)
{
  ...
  *index = m.function.n_funcdecl + m.importsection.functions;
  ...
  if(name.len > 0)
    if(err = WatString(env, body.debug.name, name))
      return err;
  ...
  if(err = AppendArray(env, sig, m.function.funcdecl, m.function.n_funcdecl))
    return err;
...
int WatParser::ParseInitializerInstruction(Queue<WatToken>& tokens, Instruction& op, bool expr)
{
  ...
  if(expr)
  {
    if(err = ParseExpression(tokens, blank, typeblank, 0))
      return err;
  }
  else
  {
  ...
  if(blank.n_body > 1)
  {
    ...
    AppendError(env, env.errors, 0, (i == blank.n_body) ? ERR_INVALID_INITIALIZER_TYPE : ERR_INVALID_INITIALIZER, "Only one instruction is allowed as an initializer");
  }
...
int WatParser::ParseMemoryDesc(MemoryDesc& m, Queue<WatToken>& tokens)
{
  return ParseResizableLimits(m.limits, tokens);
}
...
int WatParser::ParseModule(Environment& env, Module& m, Queue<WatToken>& tokens, StringRef name, WatToken& internalname)
{
  ...
  auto procRef = [](WatParser& s, Module& mod, varuint32 e) {
    if(s.deferred[0].func < mod.importsection.functions || s.deferred[0].func >= mod.code.n_funcbody + mod.importsection.functions)
  ...
  while(state.deferred.Size() > 0)
  {
    ...
    case OP_global_get:
    case OP_global_set:
      err = procRef(state, m, state.GetFromHash(state.globalhash, state.deferred[0].t));
      break;
    case OP_call:
      err = procRef(state, m, state.GetFromHash(state.funchash, state.deferred[0].t));
      break;
    default:
      return ERR_WAT_INVALID_TOKEN;
    }
