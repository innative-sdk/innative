...
namespace innative {
  namespace parse {
    ...
    IR_ERROR ParseByteArray(Stream& s, ByteArray& section, bool terminator)
    {
      varuint32 n;
    ...
    IR_ERROR ParseInstruction(Stream& s, Instruction& ins)
    {
      ...
      case OP_i64_store16:
      case OP_i64_store32:
        ins.immediates[0]._memflags = s.ReadVarUInt32(err);
      ...
      while(err >= 0 && !s.End())
      {
        ...
        case WASM_SECTION_CUSTOM:
          ...
          if(err == ERR_SUCCESS && !strcmp(m.custom[curcustom].name.str(), "name"))
            ParseNameSection(s, s.pos + payload - m.custom[curcustom].name.size(), m);
          else
            s.pos += payload - m.custom[curcustom].name.size(); // Skip over the custom payload, minus the name
          break;
        default:
