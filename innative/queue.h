...
#ifndef __QUEUE_H__IR__
#define __QUEUE_H__IR__
...
namespace innative {
  ...
  public:
    ...
    const T& operator[](size_t i) const { assert(_pos + i < _array.size()); return _array[_pos + i]; }
    T& operator[](size_t i) { assert(_pos + i < _array.size()); return _array[_pos + i]; }
