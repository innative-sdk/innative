...
typedef struct IN__EXPORTS
{
  ...
  /// contains the module. \param size The length of the memory that the data pointer points to, or zero if the data pointer
  /// is actually a UTF8 encoded null terminated string. \param name A name to use for the module. If the module data does
  /// not contain a name, this will be used. \param err A pointer to an integer that recieves an error code should the
  /// function fail. Not valid until FinalizeEnvironment() is called.
  ...
  /// parameter should be null for standard C functions. \param export_name The name of the function to add to the
  ...
  /// webassembly modules can call. \param env The environment to modify. \param tag An IN_EMBEDDING_TAGS enumeration option
  /// that provides an optional hint for the type of file. To automatically choose, set to 0. \param data Either a pointer
  /// to memory, or a UTF8 encoded null-terminated string pointing to a file. \param size The length of the memory that the
  ...
  /// ensures all configuration data is loaded. \param env The environment to finalize
  ...
  /// and other exported symbols. \param file the path of the file to load.
  ...
  /// signature, you must cast it manually to the correct function type. \param assembly A pointer to a webassembly binary
  /// loaded by LoadAssembly. \param module_name The name of the module the function is exported from. \param function The
  ...
  /// if you know how many entries the table has. \param assembly A pointer to a webassembly binary loaded by LoadAssembly.
  ...
  /// is mutable (but this function cannot determine whether it was intended to be mutable). \param assembly A pointer to a
  /// webassembly binary loaded by LoadAssembly. \param module_name The name of the module the global is exported from.
  ...
  /// pointer. \param env The environment to clear. \param m An optional module whose compilation cache will be cleared. If
  /// this is a null pointer, instead clears the entire cache of the environment.
...
/// contains the script. \param sz The length of the memory that the data pointer points to, or zero if the data pointer is
/// actually a UTF8 encoded null terminated string. \param env The environment to execute the script with. This environment
/// can potentially be modified by the script being executed. \param always_compile By default, this function only performs
/// a compilation when necessary to call a function. If this is set to true,
...
/// in env->objpath
IN_COMPILER_DLLEXPORT extern int innative_compile_script(const uint8_t* data, size_t sz, Environment* env,
                                                         bool always_compile, const char* output);
...
/// shared library. \param out The output file that will store the compilation result. \param log A C FILE* stream that
